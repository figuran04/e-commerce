...
class ServiceHelper
{
    ...
    {
        // Maps service name to its API prefix on the Nginx gateway
        $map = [
            'auth'     => 'http://nginx/api/auth/',
            'products' => 'http://nginx/api/products/',
            'orders'   => 'http://nginx/api/orders/',
            'cart'     => 'http://nginx/api/cart/',
        ];
        return $map[$service] ?? 'http://nginx/api/' . $service . '/';
    }
    ...
    public static function fetchProducts(array $productIds): array
    {
        ...
        $res = self::call('products', 'bulk', 'POST', ['ids' => $productIds]);
    ...
    {
        $res = self::call('products', 'decrease-stock', 'POST', [
    ...
    {
        $res = self::call('products', 'increase-stock', 'POST', [
            'product_id' => $productId,
            'quantity' => $quantity
        ]);
        return isset($res['status']) && $res['status'] === 'success';
    ...
    public static function fetchProductWithFallback(int $productId): ?array
    {
        ...
        // Try API first
        $res = self::call('products', 'bulk', 'POST', ['ids' => [$productId]]);
        if (isset($res['status']) && $res['status'] === 'success' && !empty($res['data'])) {
