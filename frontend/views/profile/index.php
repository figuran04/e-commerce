    ...
    <?php if ($profile_id == $_SESSION['user_id']) : ?>
      <div class="col-span-2 p-6 text-sm bg-white border border-gray-200 rounded-lg">
        <h3 class="mb-3 text-lg font-semibold">
          <a href="/views/orders/user_orders.php">Pesanan Saya</a>
        </h3>
        <ul class="grid grid-cols-4 gap-3">
          <li>
            <a href="/views/orders/user_orders.php?status=Dipesan" class="flex flex-col text-sm text-center tab-link">
              <i class="text-4xl ph ph-package"></i>
              Dipesan
            </a>
          </li>
          <li>
            <a href="/views/orders/user_orders.php?status=Dikirim" class="flex flex-col text-sm text-center tab-link">
              <i class="text-4xl ph ph-truck"></i>
              Dikirim
            </a>
          </li>
          <li>
            <a href="/views/orders/user_orders.php?status=Selesai" class="flex flex-col text-sm text-center tab-link">
              <i class="text-4xl ph ph-check-circle"></i>
              Selesai
            </a>
          </li>
          <li>
            <a href="/views/orders/user_orders.php?status=Dibatalkan" class="flex flex-col text-sm text-center tab-link">
...
<div id="logoutModal" class="fixed inset-0 bg-[rgba(0,0,0,0.5)] backdrop-blur items-center justify-center z-50 hidden">
  <div class="p-6 bg-white rounded-lg shadow-lg w-72">
    ...
    <div class="flex justify-end gap-2">
      ...
      <button onclick="handleLogout()" class="px-4 py-1 text-white bg-red-500 rounded hover:bg-red-600">Keluar</button>
    </div>
  </div>
</div>
...
<script>
  ...
  async function handleLogout() {
    if (typeof Auth !== 'undefined') {
      ...
      form.method = 'POST';
      form.action = '../../controllers/auth/logout_handler.php';
