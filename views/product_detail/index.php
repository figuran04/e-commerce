...
<div class="flex flex-col md:flex-row items-center gap-6 mt-6">
  ...
  <div>
    <a href="../profile?id=<?= htmlspecialchars($product['user_id']) ?>" class="text-lime-600 font-semibold hover:underline">user<?= htmlspecialchars($product['user_id']) ?></a>
    ...
    <p class="text-lime-600 font-bold text-2xl">Rp<?= number_format($product['price'], 0, ',', '.'); ?></p>
    ...
    <form action="../../controllers/cart/add_to_cart.php" method="POST" class="mt-4">
      ...
      <button type="submit" class="ml-2 px-4 py-2 bg-lime-600 text-white rounded">Masukkan ke Keranjang</button>
    </form>
  </div>
</div>
