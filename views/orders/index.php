    ...
    <ul class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-1">
      <?php foreach ($order['items'] as $item): ?>
        <li class="hover:bg-gray-200 transition-all rounded-lg p-2 border border-gray-200 bg-white group">
          <a href="/5/e-commerce/views/product_detail/?id=<?= $item['product_id'] ?>" class="flex items-center space-x-4">
            <img src="/5/e-commerce/uploads/<?= $item['image'] ?>" alt="<?= $item['name'] ?>" class="w-24 h-24 object-cover rounded-lg group-hover:scale-105 transition-all" />
